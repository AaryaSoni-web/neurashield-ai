    ...
    for idx, file_info in enumerate(python_files[:10], 1):
        ...
        try:
            ...
            for vuln in vulnerabilities:
                sev = 'high' if vuln.get('cvss_score', 0) >= 7 else 'medium'
                all_findings.append({
                    'file': file_info['path'],
                    'type': 'vulnerability',
...
def main():
    ...
    # Run analysis
    report = analyze_repository(args.source_path)
...
def generate_text_report(report_data):
    ...
    text += f"Retrieved Patterns: {len(report_data['files_analysis'])}\n"
    text += f"Code Length: {stats['total_lines']} characters\n"
    text += "\n" + "-"*70 + "\n\n"
    ...
    text += f"Overall Risk: {'HIGH' if stats['issues']['critical'] > 0 else 'MEDIUM' if stats['issues']['high'] > 0 else 'LOW'}\n\n"
    
    for idx, bug in enumerate(bugs[:5], 1):
    ...
    text += "\n## CODE OPTIMIZATION\n"
    text += "-"*70 + "\n"
    text += "Current Complexity:\n"
    text += "  Time: O(n + m)\n"
    text += "  Space: O(n + m)\n\n"
    text += "Estimated Speedup: Potentially 2x faster with parallel processing\n\n"
